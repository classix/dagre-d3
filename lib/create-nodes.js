...
function createNodes(selection, g, shapes) {
  ...
  svgNodes.each(function(v) {
    ...
    var shapeSvg = shape(root, bbox, node, dir).classed("label-container", true);
    ...
    ["predecessorsExpandButton", "successorsExpandButton"].forEach(function (prefix) {
      if (node[prefix + "Status"] && node[prefix + "Status"] !== "alwaysHidden") {
        var expandButton = util.drawExpandButton(thisGroup, node[prefix + "X"], node[prefix + "Y"], node.expandButtonRadius, node[prefix + "Collapsed"], node.expandButtonStyle, node.expandButtonHoverStyle, prefix === "predecessorsExpandButton");
