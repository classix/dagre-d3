...
function createEdgePaths(selection, g) {
  var previousPaths = selection.selectAll("g.edgePath")
    .data(g.edges().filter(function (edgeId) { return !g.edge(edgeId).isHidden; }), function(e) { return util.edgeToId(e); })
    .classed("update", true);
  ...
  svgPaths.sort(function (e1,e2) {
    var edge1 = g.edge(e1) || {};
    var edge2 = g.edge(e2) || {};
    return _.defaultTo(edge1.zIndex, 0) - _.defaultTo(edge2.zIndex, 0);
  }).each(function(e) {
...
function enter(svgPaths, g) {
  ...
  var svgPathsEnter = svgPaths.enter().filter(function (e) { return (!g.node(e.v).isHidden && !g.node(e.w).isHidden); })
    .append("g")
      .attr("class", "edgePath")
      .style("opacity", 0);
